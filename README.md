...
- **1 minute is the smallest scheduling unit.** Every planned-time write (`update_task`, `bulk_update_tasks`, `batch_update_project`, `plan_tasks_for_today(plan_from_now)`) is **floored to the whole minute** on the server before it reaches SP — sub-minute timestamps (e.g. raw `Date.now()` at `15:45:46.715`) are rounded DOWN. SP renders times at HH:mm only, so an unfloored `due_with_time` could produce a seconds-level overlap that every rendered time denies (see #13); normalization at the write boundary makes back-to-back blocks safe no matter what the agent passes in.
