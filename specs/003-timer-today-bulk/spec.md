...
- `start_task` uses `dispatchAction` with NgRx action `[Task] SetCurrentTask` with `{ id }` (no native PluginAPI method exists)
- `stop_task` uses `dispatchAction` with NgRx action `[Task] SetCurrentTask` with `{ id: null }` (unsetCurrentTask is not whitelisted)
